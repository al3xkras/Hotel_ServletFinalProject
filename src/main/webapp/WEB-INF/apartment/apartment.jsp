...
<head>
    <meta charset="UTF-8">
    <title>Apartment ${requestScope.apartment.name}</title>
...
<body>
    <div class="custom-navbar">
        <a class="active" href="${pageContext.request.contextPath}/app/apartments">
            <fmt:message key="navbar.all_apartments"/>
        </a>
        ...
        <div class="custom-navbar-dropdown" style="float: right">
            ...
            <div class="custom-dropdown-content" style="left: 80%; width: 19.5%">
                <a href="${pageContext.request.contextPath}/app/${sessionScope.user.userType.name().equalsIgnoreCase("user")?"user":"admin"}/personal_data">
                    <fmt:message key="navbar.personal_data"/>
                </a>
            </div>
        </div>
    ...
    <div style="margin-bottom: 100px">
        <h1 style="text-align: center; margin-top: 5%; margin-bottom: 30px">${requestScope.apartment.name}</h1>
        ...
        <div class="d-flex justify-content-left" style="margin: 5px; min-height: 40%;">
            ...
            <div class="d-flex flex-column" style="margin: 5px; min-height: 40%;">
                <div>
                    <label><fmt:message key="new_apartment.class"/>:</label>
                    <label class="${requestScope.apartment.apartmentClass.htmlClass}">
                        <fmt:message key="${requestScope.apartment.apartmentClass.resName}"/>
                    </label><br/>
                </div>
                <div>
                    <label>
                        <fmt:message key="new_apartment.status"/>:
                    </label>
                    <label class="${requestScope.apartment.status.htmlClass}">
                        <fmt:message key="${requestScope.apartment.status.resName}"/>
                    </label><br/>
                </div>

                <label><fmt:message key="new_apartment.places"/>: ${requestScope.apartment.places}
                </label><br/>
                <label><fmt:message key="new_apartment.price"/>: ${requestScope.apartment.price}
                </label><br/>
                <c:if test='${!requestScope.apartment.status.name().equalsIgnoreCase("available")}'>
                    <label style="font-size: 110%; color: red" >
                        <fmt:message key="message.apartment.unavailable"/>
                    </label><br/>
                </c:if>
                ...
                <c:if test="${requestScope.apartment.status.name().equalsIgnoreCase('available')}">
                    <div class="d-flex flex-row-reverse">
                        <button type="button"
                            ...
                            <fmt:message key="apartment.create_reservation"/>
                        </button>
                    </div>
                </c:if>
        ...
        <dialog id="confirmation_dialog">
            ...
            <form action="${pageContext.request.contextPath}/app/apartments/apartment/${requestScope.apartment.id}" method="post">

                <div class="form-group">
                    <label for="fromDate"><fmt:message key="reservation.form.from_date"/></label>
                    <input type="date" id="fromDate" name="fromDate" required>
                </div>

                <div class="form-group">
                    <label for="toDate"><fmt:message key="reservation.form.to_date"/></label>
