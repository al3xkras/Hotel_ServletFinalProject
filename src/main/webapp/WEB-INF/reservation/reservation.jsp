...
<body>
    ...
    <div th:if="${isCompleted and (userAccount==null or !userAccount)}">
        <h1 style="text-align: center; margin-bottom: 30px" th:text="#{reservation.confirm}"></h1>
        ...
        <div class="d-flex justify-content-left" style="margin: 5px; min-height: 40%;">
            ...
            <div class="d-flex flex-column" style="margin: 5px; min-height: 40%;">
                <label th:text="#{new_apartment.class}+': '+${reservation.apartmentClass}"></label><br/>
                <label th:text="#{new_apartment.places}+': '+${reservation.places}"></label><br/>
                <label th:text="#{new_apartment.price}+': '+${reservation.apartmentPrice}"></label><br/>
                ...
                <div class="d-flex flex-row-reverse">
                    <button type="button"
                            ...
                            th:text="#{reservation.confirm}"></button>

                    <button type="button" class="btn btn-outline-danger"
                            style="margin-top: 15px; margin-right: 5%; padding: 10px"
                            onclick="discardReservationAdmin()"
                            th:text="#{reservation.discard}"></button>
                </div>

            </div>
        </div>
    </div>
    <div th:if="${isCompleted and (userAccount!=null and userAccount)}">
        <h1 style="text-align: center; margin-bottom: 30px" th:text="#{reservation.header}"></h1>
        ...
        <div class="d-flex justify-content-left" style="margin: 5px; min-height: 40%;">
            ...
            <div class="d-flex flex-column" style="margin: 5px; min-height: 40%;">
                ...
                <div class="d-flex flex-row">
                    <label th:text="#{reservation.status}+': '"></label>
                    <label th:text="#{${reservation.reservationStatus.resourceName}}"
                           th:class="${reservation.reservationStatus.htmlClass}"
                            style="margin-left: 5px"></label><br/>
                </div>

                <div class="d-flex flex-row">
                    <label th:text="#{reservation.payment_status}+': '"></label>
                    <label th:text="#{${reservation.paid?'reservation.paid':'reservation.unpaid'}}"
                           th:class="${reservation.paid?'reservation-paid':'reservation-unpaid'}"
                            style="margin-left: 5px"></label>
                </div><br/>

                <div class="d-flex flex-row-reverse" th:if="${!reservation.expired}">
                    <button type="submit" class="btn btn-outline-primary"
                            th:if="${!reservation.paid and reservation.reservationStatus.name().equalsIgnoreCase('reserved')}"
                            th:onclick="'window.location = \'/user/reservation/'+${reservation.id}+'/make_payment\';'"
                            style="margin-top: 15px; margin-right: 5%; padding: 10px" th:text="#{reservation.make_payment}"></button>

                    <button type="button"
                            class="btn btn-outline-primary"
                            th:if="${reservation.reservationStatus.canConfirm}"
                            onclick="confirmReservationUser()"
                            th:text="#{reservation.confirm}"
                            style="margin-top: 15px; margin-right: 5%; padding: 10px"></button>

                    <button type="button" class="btn btn-outline-danger"
                            style="margin-top: 15px; margin-right: 5%; padding: 10px"
                            th:if="${reservation.reservationStatus.canDiscard}"
                            onclick="discardReservationUser()"
                            th:text="#{reservation.discard}"></button>
                </div>
    ...
    <div th:if="${!isCompleted}">
        <h1 style="text-align: center; margin-bottom: 30px" th:text="#{reservation.matching_apartments}"></h1>
        ...
        <table id="apartments">
            ...
            <tr id="list_item" class="align-middle"
                th:each="apartment : ${matchingApartments}"
                th:onclick="'window.location = \'/admin/reservation/'+${reservation.id}+'/select/'+${apartment.id}+'\''">

                <td id="apartmentId" class="align-middle"  th:text="${apartment.id}"></td>
                <td id="apartmentName" class="align-middle" th:text="${apartment.name}"></td>
                <td id="apartmentPlaces" class="align-middle" th:text="${apartment.places}"></td>
                <td id="apartmentClass" class="align-middle" th:text="${apartment.apartmentClass}"></td>
                <td id="apartmentStatus" class="align-middle" th:text="${apartment.status}"></td>
                <td id="apartmentPrice" class="align-middle" th:text="${apartment.price}"></td>
            </tr>
        </table>
        ...
        <dialog id="select-apartment">
            <div class="modal-header">
                <h5 class="modal-title" th:text="#{dialog.confirmation}"></h5>
            </div>

            <div class="form-body">
                <p th:text="#{apartment.assign1}+${reservation.apartmentId}+#{apartment.assign2}"></p>
            </div>

            <form method="post" th:action="@{'/admin/reservation/'+${reservation.id}+'/confirm/'+${reservation.apartmentId}}">
                <button type="button" class="btn btn-primary" id="cancel-select-apartment" th:text="#{dialog.cancel}" style="margin: 5px"></button>
                <button type="submit" class="btn btn-secondary" style="float: right; margin: 5px" th:text="#{dialog.confirm}" ></button>
            </form>
    ...
    <dialog id="discard-reservation-admin" style="background: white">
        ...
        <div class="form-body">
            <p th:text="#{dialog.discard_reservation}"></p>
        </div>

        <form action="#" th:action="@{'/admin/reservation/'+${reservation.id}+'/cancel'}" method="post">
            <button type="button" class="btn btn-secondary" onclick="hideDiscardReservationAdmin()"
                    th:text="#{dialog.cancel}" style="margin: 5px"></button>
            <button type="submit" class="btn btn-outline-danger" style="float: right; margin: 5px" th:text="#{dialog.drop}"></button>
        </form>
    </dialog>
    ...
    <dialog id="discard-reservation-user" style="background: white">
        ...
        <form action="#" th:action="@{'/user/reservation/'+${reservation.id}+'/cancel'}" method="post">
            <button type="button" class="btn btn-secondary" onclick="hideDiscardReservationUser()" th:text="#{dialog.cancel}" style="margin: 5px"></button>
            <button type="submit" class="btn btn-outline-danger" style="float: right; margin: 5px" th:text="#{dialog.drop}"></button>
        </form>
    </dialog>
    ...
    <dialog id="confirm-reservation-user" style="background: white">
        ...
        <div class="form-body">
            <p th:text="#{dialog.confirm_reservation}"></p>
        </div>

        <form action="#" th:action="@{'/user/reservation/'+${reservation.id}+'/confirm'}" method="post">
            <button type="button" class="btn btn-secondary" onclick="hideConfirmReservationUser()" th:text="#{dialog.cancel}" style="margin: 5px"></button>
            <button type="submit" class="btn btn-outline-primary" style="float: right; margin: 5px" th:text="#{dialog.confirm}"></button>
        </form>
    </dialog>
    ...
    <dialog id="confirm-reservation-admin" style="background: white">
        ...
        <form method="post" th:action="@{'/admin/reservation/'+${reservation.id}+'/confirm'}">
            <button type="button" class="btn btn-secondary" onclick="hideConfirmReservationAdmin()" th:text="#{dialog.cancel}" style="margin: 5px"></button>
            <button type="submit" class="btn btn-outline-primary" style="float: right; margin: 5px" th:text="#{dialog.confirm}"></button>
        </form>
    </dialog>
    ...
    <script type="text/javascript" th:src="@{/js/reservation/reservation.js}"></script>
</body>
</html>